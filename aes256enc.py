import os
from Cryptodome.Cipher import AES
from Crypto.Util.Padding import pad

def encrypt_shell(buf):

#convert shellcode to bytes
	shellcode=bytes(buf)

#generate AES key and IV.
	key = os.urandom(32)
	iv = os.urandom(16)

# Encrypt shellcode with AES

	cipher = AES.new(key, AES.MODE_CBC, iv)

	encrypted_shellcode = cipher.encrypt(pad(shellcode, AES.block_size))


# Calculate Size
	encrypted_shellcode_size = len(encrypted_shellcode)

#format keys for c# in xx-xx format.
	key_formatted = '-'.join(f"{b:02X}" for b in key)
	iv_formatted = '-'.join(f"{b:02X}" for b in iv)

# Return results for c# compatibility
	return {
		"key": key_formatted,
		"iv": iv_formatted,
		"encrypted_shellcode":{
			"data": ', '.join(f"0x{b:02x}" for b in encrypted_shellcode),
			"size": encrypted_shellcode_size
			}
		}

buf = [ 

	 ]


result = encrypt_shell(buf)


print("C# AES Key:")
print(result["key"])

print("\nC# IV Key:")
print(result["iv"])

print("\nC# Encrypted Shell:")
print(f"byte[] buf = new byte[{result['encrypted_shellcode']['size']}] {{ {result['encrypted_shellcode']['data']} }};")



